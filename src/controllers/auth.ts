...
export const login = async (req: Request, res: Response) => {
  ...
  const user = (
    await query(
      'SELECT id, username, password, email, image_url, is_admin FROM users WHERE email = $1 LIMIT 1;',
      [email]
    )
  ).rows[0];
...
export const register = async (req: Request, res: Response) => {
  ...
  const user = (await query(
    'INSERT INTO users (username, email, password) VALUES ($1, $2, $3) RETURNING id, username, email, image_url, is_admin;',
